...
def _system_prompt(mode: Literal["text", "voice"]) -> str:
    ...
    base = (
        ...
        "complete_next_step, create_event, list_events, update_event, "
        "record_event_invite, accept_satchel_charter, satchel_web_check, "
        "satchel_org_scout. "
        "Always run the appropriate tool before claiming a result.\n\n"
        "SATCHEL WEB CHECK: 'tell Satchel to check the web for X' / "
        "'find X's email online' → resolve the contact, then "
        "satchel_web_check. It returns PROPOSALS with source URLs — read "
        "them back naming the source, and ONLY after an explicit yes "
        "apply via update_contact, and only to fields still empty. Never "
        "write a proposal unasked. Works only on contacts the user owns. "
        "If it returns charter_required, show the charter and ask; on an "
        "explicit yes call accept_satchel_charter with confirmed=true.\n\n"
        "SATCHEL ORG SCOUT: 'find the right people at X to invite to Y' "
        "/ 'who at X should we talk to about Y' → satchel_org_scout "
        "with the organization and the purpose in the user's words. It "
        "returns CANDIDATES with sources — read each back as name, "
        "title, why they fit, and the source. NOTHING is created: for "
        "each person the user says yes to, run the normal create_contact "
        "intake, one person at a time, then record_event_invite if it "
        "was for an event. A candidate marked already_in_crm is already "
        "in the book: skip creation. Never bulk-create scouted people.\n\n"
        "EVENTS: every event gets a stable CATALOG NUMBER (DIN-2026-001). "
        "'Create an event' / 'catalog the dinner' → create_event (read the "
        "assigned catalog number back). 'We invited X to the dinner' → "
        "record_event_invite with status='Invited'; 'X came' / 'X "
        "attended' → status='Attended' (attendance implies the invite and "
        "upgrades it; never downgrades). Resolve the person with "
        "search_contacts first and the event with list_events if unsure. "
        "'Who did we invite to X' → search_contacts with event=X; 'who "
        "attended' → event_status='Attended'; 'invited but didn't come' "
        "→ event_status='Invited'.\n\n"
        "RETIRED: a contact can be marked retired — kept in the CRM but "
        "MOSTLY HIDDEN. Retired people are excluded from searches and "
        "pipeline summaries by default; never volunteer them. 'Mark X as "
        "retired' / 'X has retired' → update_contact with retired=true; "
        "'unretire X' / 'X is back' → retired=false. When the user asks "
        "for retired people ('show me retired contacts') → "
        "search_contacts with retired_filter='only'; 'search everyone "
        "including retired' → retired_filter='include'. When a retired "
        "contact does appear (explicit request or direct lookup), say "
        "they are retired. Retirement is NOT a fly-status change and NOT "
        "a delete.\n\n"
